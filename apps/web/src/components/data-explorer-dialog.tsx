                  ...
                  <TableBody>
                    {records.map((record) => (
                      <TableRow key={record.id} className="group hover:bg-muted/50">
                        <TableCell className="font-mono text-xs text-muted-foreground border-r group-hover:text-foreground transition-colors">
                          {record.id.split('-')[0]}...
                        </TableCell>
                        ...
                        {rawHeaders.map(header => {
                          ...
                          return (
                            <TableCell key={`${record.id}-${header}`} className="text-foreground max-w-[200px] truncate" title={displayVal}>
                              {displayVal}
                            </TableCell>
                          );
                        })}
                ...
                <Table>
                  <TableHeader className="bg-muted/50 sticky top-0 z-10 shadow-[0_1px_2px_0_rgba(255,255,255,0.05)] backdrop-blur-sm">
                    <TableRow className="hover:bg-transparent border-border">
                      <TableHead className="w-[120px] font-semibold text-xs text-primary border-r border-border">_status</TableHead>
                      <TableHead className="w-[180px] font-semibold text-xs text-primary border-r border-border">_transformed_at</TableHead>
                      {canonicalHeaders.map(header => (
                        <TableHead key={header} className="font-semibold text-xs text-primary">{header}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {records.map((record) => {
                      ...
                      return (
                        <TableRow key={cRec.id} className="hover:bg-muted/30 border-border transition-colors">
                          ...
                          <TableCell className="text-xs text-muted-foreground border-r border-border font-medium">
                          ...
                          {canonicalHeaders.map(header => {
                            ...
                            const displayVal = typeof val === 'object' ? JSON.stringify(val) : String(val ?? '-');
                            return (
                              <TableCell key={`${cRec.id}-${header}`} className="text-foreground font-medium max-w-[200px] truncate" title={displayVal}>
                                {displayVal}
                              </TableCell>
                            );
                          })}
