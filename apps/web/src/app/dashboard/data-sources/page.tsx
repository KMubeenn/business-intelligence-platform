  ...
  return (
    <div className="flex flex-col gap-8 w-full pt-4">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
        {loading ? (
          <div className="py-12 text-center text-muted-foreground text-sm font-medium">Loading connections...</div>
        ) : error ? (
          ...
          <div className="w-full">
            <div className="grid grid-cols-5 border-b border-border bg-muted/50 p-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <div className="divide-y divide-border">
              {dataSources.map((ds) => (
                <div key={ds.id} className="grid grid-cols-5 items-center p-4 text-sm hover:bg-muted/50 transition-colors">
                  <div className="col-span-2 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground font-mono mt-0.5">{ds.id.split('-')[0]}...</div>
                    </div>
                  </div>
                  <div className="font-medium text-muted-foreground">{ds.type}</div>
                  <div>
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold border ${ds.status === 'ACTIVE'
                      ...
                      <span className={`mr-1.5 h-1.5 w-1.5 rounded-full ${ds.status === 'ACTIVE' ? 'bg-primary' : 'bg-muted-foreground'}`}></span>
                      {ds.status}
                    </span>
                  </div>
                  <div className="flex justify-end gap-2">
                    <Link href={`/dashboard/data-sources/${ds.id}`}>
                      <Button variant="ghost" size="sm" className="h-8 text-muted-foreground hover:text-foreground hover:bg-accent">
